...
class simple_fv {
	...
	private function validate($type, $val, $index_val, $label, $test=false) {
		...
		if($type == 'min_val') {
			if((float)$index_val < (int)$val) {
				if(!$test) {
					...
					$this->custom_error[$label] = __('minimum value is', 'ag_ml').' '.$val;
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'max_val') {
			if((float)$index_val > (int)$val) {
				if(!$test) {
					#$this->errors['value'][] = $label;
					$this->custom_error[$label] = __('maximum value is', 'ag_ml').' '.$val;
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'min_len') {
			if(strlen($index_val) < (int)$val) {
				if(!$test) {
					...
					$this->custom_error[$label] = __('must be at least of', 'ag_ml').' '.$val.' '.__('characters', 'ag_ml');
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'max_len') {
			if(strlen($index_val) > (int)$val) {
				if(!$test) {
					#$this->errors['lenght'][] = $label;
					$this->custom_error[$label] = __('maximum', 'ag_ml').' '.$val.' '.__('characters allowed', 'ag_ml');
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'right_len') {
			if(strlen($index_val) != (int)$val) {
				if(!$test) {
					#$this->errors['lenght'][] = $label;
					$this->custom_error[$label] = __('must be long', 'ag_ml').' '.$val.' '.__('characters', 'ag_ml');
				}
				return false;
			}	
			else {return true;}
		...
		if($type == 'min_array') {
			if(!is_array($index_val) || count($index_val) < $val) {
				if(!$test) {
					...
					$this->custom_error[$label] = __('Select').' '.$val.' '.__('options', 'ag_ml');
				}
				return false;	
			}
		}
		...
		if($type == 'max_array' && is_array($index_val)) {
			if(count($index_val) > $val) {
				if(!$test) {
					#$this->errors['array_count'][] = $label;
					$this->custom_error[$label] = __('Maximum', 'ag_ml').' '.$val.' '.__('options allowed', 'ag_ml');
				}
				return false;	
			}
		}
	...
	private function errorTranslate($type, $label_array) {
		...
		switch($type) {
			case 'required' : 
				(count($label_array) > 1) ? $message .= __('are required', 'ag_ml') : $message .= __('is required', 'ag_ml');
				break;
			
			case 'wrong_int' : 
				(count($label_array) > 1) ? $message .= __('are not valid integers', 'ag_ml') : $message .= __('is not a valid integer', 'ag_ml');
				break;
				
			case 'wrong_float' : 
				(count($label_array) > 1) ? $message .= __('are not valid floating numbers', 'ag_ml') : $message .= __('is not a valid floating number', 'ag_ml');
				break;	
				
			case 'wrong_mail' : 
				(count($label_array) > 1) ? $message .= __('are not valid e-mail addresses', 'ag_ml') : $message .= __('is not a valid e-mail address', 'ag_ml');
				break;		
				
			case 'wrong_date' : 
				(count($label_array) > 1) ? $message .= __('are not valid dates', 'ag_ml') : $message .= __('is not a valid date', 'ag_ml');
				break;		
				
			case 'wrong_time' : 
				(count($label_array) > 1) ? $message .= __('are not valid times', 'ag_ml') : $message .= __('is not a valid time', 'ag_ml');
				break;
				
			case 'wrong_url' : 
				(count($label_array) > 1) ? $message .= __('are not valid urls', 'ag_ml') : $message .= __('is not a valid url', 'ag_ml');
				break;	
				
			case 'wrong_hex' : 
				(count($label_array) > 1) ? $message .= __('are not valid hexadecimal colors', 'ag_ml') : $message .= __('is not a valid hexadecimal color', 'ag_ml');
				break;	
			
			case 'wrong_ip' : 
				(count($label_array) > 1) ? $message .= __('are not valid IP addresses', 'ag_ml') : $message .= __('is not a valid IP address', 'ag_ml');
				break;	
				
			case 'wrong_zip' : 
				(count($label_array) > 1) ? $message .= __('are not valid ZIP codes', 'ag_ml') : $message .= __('is not a valid ZIP code', 'ag_ml');
				break;
				
			case 'wrong_tel' : 
				(count($label_array) > 1) ? $message .= __('are not valid telephone numbers', 'ag_ml') : $message .= __('is not a valid telephone number', 'ag_ml');
				break;		
					
			case 'wrong_type' : $message .= __('invalid data inserted', 'ag_ml');
				break;	
				
			case 'allowed' : 
				(count($label_array) > 1) ? $message .= __('values are not between the allowed', 'ag_ml') : $message .= __('value is not between the allowed', 'ag_ml');
				break;
				
			case 'forbidden' : 
				(count($label_array) > 1) ? $message .= __('values are between the forbidden', 'ag_ml') : $message .= __('values is between the forbidden', 'ag_ml');
				break;
				
			case 'not_equal' : $message .= __("the value doesn't match", 'ag_ml'); 
				break;
				
			case 'size' : $message .= __('file size is wrong', 'ag_ml');
				break;						
		}	
