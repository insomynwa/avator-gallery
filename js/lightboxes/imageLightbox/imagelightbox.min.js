...
d=d.add(a(this))});this.switchImageLightbox=function(a){var b=d.eq(a);if(b.length){var c=d.index(e);e=b;v(a<c?"left":"right")}return this};this.quitImageLightbox=function(){u();return this};return this};ag_activityIndicatorOn=function(){a('<div id="imagelightbox-loading"><div></div></div>').appendTo("body")};ag_activityIndicatorOff=function(){a("#imagelightbox-loading").remove()};ag_overlayOn=function(){a('<div id="imagelightbox-overlay"></div>').appendTo("body")};ag_overlayOff=function(){a("#imagelightbox-overlay").remove()};
ag_closeButtonOn=function(b){a('<button type="button" id="imagelightbox-close" title="Close"></button>').appendTo("body").on("click touchend",function(){a(this).remove();b.quitImageLightbox();return!1})};ag_closeButtonOff=function(){a("#imagelightbox-close").remove()};ag_captionOn=function(){var b=a('a[href="'+a("#imagelightbox").attr("src")+'"] img').attr("alt");0<b.length&&a('<div id="imagelightbox-caption">'+b+"</div>").appendTo("body")};ag_captionOff=function(){a("#imagelightbox-caption").remove()};
ag_navigationOn=function(b,d){var e=a(d);if(e.length){for(var c=a('<div id="imagelightbox-nav"></div>'),g=0;g<e.length;g++)c.append('<button type="button"></button>');c.appendTo("body");c.on("click touchend",function(){return!1});var f=c.find("button");f.on("click touchend",function(){var c=a(this);e.eq(c.index()).attr("href")!=a("#imagelightbox").attr("src")&&b.switchImageLightbox(c.index());f.removeClass("active");f.eq(c.index()).addClass("active");return!1}).on("touchend",function(){return!1})}};
ag_navigationUpdate=function(b){var d=a("#imagelightbox-nav button");d.removeClass("active");d.eq(a(b).filter('[href="'+a("#imagelightbox").attr("src")+'"]').index(b)).addClass("active")};ag_navigationOff=function(){a("#imagelightbox-nav").remove()};ag_arrowsOn=function(b,d){var e=a('<button type="button" class="imagelightbox-arrow imagelightbox-arrow-left"></button><button type="button" class="imagelightbox-arrow imagelightbox-arrow-right"></button>');e.appendTo("body");e.on("click touchend",function(c){c.preventDefault();
c=a(this);var e=a(d+'[href="'+a("#imagelightbox").attr("src")+'"]').index(d);c.hasClass("imagelightbox-arrow-left")?(--e,a(d).eq(e).length||(e=a(d).length)):(e+=1,a(d).eq(e).length||(e=0));b.switchImageLightbox(e);return!1})};ag_arrowsOff=function(){a(".imagelightbox-arrow").remove()}})(jQuery,window,document);